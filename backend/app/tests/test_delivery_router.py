...
def test_get_delivery_invalid_format():
...
def test_update_delivery_status_invalid_payload():
    response = client.put("/deliveries/testid/status", json={})
    assert response.status_code == 422

def test_update_delivery_status_not_found():
    response = client.put("/deliveries/nonexistent/status",json={"delivery_status": "Order Delivered"})
    assert response.status_code == 404
